 ...
 * an identical body.
 *
 * <h2>Status codes for money failures</h2>
 *
 ...
 *   <li><strong>422</strong> — the request was right, but the account cannot satisfy it
 *       (insufficient funds). Resending the same request after a top-up will work.</li>
 *   <li><strong>409</strong> — the request was right, but the wallet's state or a
 *       concurrent writer got in the way. Resending unchanged may well work.</li>
 * </ul>
 */
...
public class GlobalExceptionHandler {
    ...
    /** A path variable that could not be converted — most often a malformed wallet UUID. */
    ...
    /**
     * The wallet cannot cover the debit.
     *
     * <p>422, not 400: nothing about the request is malformed. Logged at info because a
     * declined payment is a normal business outcome, not a fault.
     */
    @ExceptionHandler(InsufficientFundsException.class)
    public ResponseEntity<ErrorResponse> handleInsufficientFunds(
            InsufficientFundsException ex, HttpServletRequest request) {
        log.info("Debit declined for insufficient funds on {}", request.getRequestURI());
        return build(HttpStatus.UNPROCESSABLE_ENTITY, ex.getMessage(), request, null);
    }

    /** Money asked to move on a FROZEN or CLOSED wallet. */
    @ExceptionHandler(WalletNotActiveException.class)
    public ResponseEntity<ErrorResponse> handleWalletNotActive(
            WalletNotActiveException ex, HttpServletRequest request) {
     ...
     *
     * <p>Distinct from {@link ConcurrentUpdateException} and more noteworthy: the money
     * paths retry these, so one arriving here came from somewhere that does not — wallet
     * creation, for instance. Logged at warn for that reason. Still a 409, and still safe
     * to retry, because the transaction rolled back.
     */
     ...
     * parameter values and — in this service — balances.
     */
